...
const { monthHue, hsvToPacked, boostContrast, buildColumn, buildGrid, decodeJpeg, decodePng } = mod;
...
// Avatar layout: a full 64-pixel avatar occupies the leftmost 8 columns and
// the heatmap shrinks to the remaining 24
...
eq("avatar layout has no heatmap in column 8", gWithAvatar[8 * 8 + 4], 0);
...
const PNG_B64 = "";

const JPG_EXPECT = [3749161,3550242,4666154,4204833,4995630,4735281,3355686,1843987,4669749,4997688,5915451,7033419,2429703,3879459,3882030,2830882,5721924,6444364,9994617,10257276,5126703,3352345,5197120,3159077,8353644,8549483,9862518,8743523,5652533,5588796,6775639,4540217,9734784,10523273,11638415,7953494,6375489,7759195,8156778,5066304,10392714,11181203,11703954,9400940,6375489,9272434,8880501,6710359,11379867,11641754,13151400,11177353,11375757,12298912,12301737,8486771,11445660,14667977,15389132,13282729,12955302,14206909,13419963,9210494];

const PNG_EXPECT = [9215194,10002913,10462949,10791655,9803698,8289673,9278401,9739477,10397157,11120103,11711468,11645666,9275788,8618106,8289659,7696494,11185892,12171247,12237552,10658741,9934739,9471615,9275264,8289141,11383000,9734041,7427395,11118761,11712703,10065303,10461088,9473156,11119816,8350313,9866373,14016753,11779273,12239312,13753836,10263711,10396596,10659257,9738140,13029082,9869988,13884649,14740217,14016752,10001835,10133421,6446429,9540508,7827817,7104869,13621221,13490921,9541284,10002092,11383488,11185851,8027009,9606817,9804195,13161699];
...
console.log("avatar decode");
{
  ...
  let maxDiff = 0;
  for (let i = 0; i < 64; i++) {
    const dr = Math.abs(((got[i] >> 16) & 0xff) - ((JPG_EXPECT[i] >> 16) & 0xff));
...
}
{
  ...
  for (let y = 0; y < h; y++) {
    for (let x = 0; x < w; x++) {
      const o = (y * w + x) * 4;
      if (x < w / 2) { rgba[o] = 255; } else { rgba[o + 1] = 255; }
      rgba[o + 3] = 255;
    }
  }
  ...
  eq("identicon downscale is box-averaged",
     String(row0), String([0xff0000, 0xff0000, 0xff0000, 0xff0000, 0x00ff00, 0x00ff00, 0x00ff00, 0x00ff00]));
